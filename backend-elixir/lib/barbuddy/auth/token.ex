defmodule BarBuddy.Auth.Token.Jwt do
  @moduledoc """
  ...
  * `allowed_algos` - a list of allowable algos
  ...
  def decode_token(mod, token, options \\ []) do
    IO.puts "Decode_token"
    err_result = with {:ok, secret_fetcher} <- fetch_secret_fetcher(mod),
      ...
      Logger.info "Verifying with secret"
      verify_result = JWT.verify_strict(secret, algos, token)

      case verify_result do
        {true, jose_jwt, _} ->
          IO.puts "Decoded Jose jwt: #{inspect jose_jwt}"
          {:ok, jose_jwt.fields}
        {false, one, two} ->
          Logger.info "Verify result failed: #{inspect one} #{inspect two}"
          {:error, :invalid_token}
      end
    else
      something -> Logger.info "Error: #{inspect something}"
    end
  end
